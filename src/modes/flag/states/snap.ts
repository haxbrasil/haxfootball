...
import { $setBallMoveable, $unlockBall } from "@modes/flag/hooks/physics";
import {
    $hideCrowdingBoxes,
    $setBallActive,
    $setBallInactive,
...
import { getTouchdownScore } from "@modes/flag/shared/rules/scoring";
import {
    $before,
    $dispose,
    $effect,
...
import * as Crowding from "@modes/flag/shared/interaction/crowding";
...
} from "@modes/flag/shared/rules/blitz";
import {
    DEFAULT_PUSHING_CONTACT_DISTANCE,
    DEFAULT_PUSHING_MIN_BACKFIELD_STEP,
    detectPushingFoul,
} from "@modes/flag/shared/interaction/pushing";
...
function getNextDriveDownState(scoringTeam: FieldTeam) {
    const nextOffensiveTeam = opposite(scoringTeam);

    return getRestartDownState(nextOffensiveTeam);
}
...
export function Snap({
        ...
        processDefensivePenaltyEvent({
            ...
            onTouchdown() {
                ...
                $next({
                    to: "PRESNAP",
                    params: {
                        downState: getNextDriveDownState(offensiveTeam),
                    },
    ...
    function $handleHandoff(frame: Frame) {
        ...
        $penalizeIllegalRun({
            playerIds: offensiveTouchers.map((player) => player.id),
            message: t`illegal handoff`,
        });
    }
    ...
    function $penalizeIllegalRun({
        ...
        $effect(($) => {
            setPlayerAvatars(playerIds, $.setAvatar, AVATARS.CLOWN);
        });

        $dispose(() => {
            $effect(($) => {
                setPlayerAvatars(playerIds, $.setAvatar, null);
            });
