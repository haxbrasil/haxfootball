...
export const TRAINING_BALL_DISC_REF = {
    LANE_0: TRAINING_TRACKED_DISC.LANE_BALL_0,
    LANE_1: TRAINING_TRACKED_DISC.LANE_BALL_1,
    LANE_2: TRAINING_TRACKED_DISC.LANE_BALL_2,
} as const;

export const TRAINING_BALL_DISC_REFS = [
    TRAINING_BALL_DISC_REF.LANE_0,
    TRAINING_BALL_DISC_REF.LANE_1,
    TRAINING_BALL_DISC_REF.LANE_2,
] as const;
...
export const TRAINING_TARGET_DISC_REFS = TRAINING_LANES.map((_, laneIndex) =>
    Array.from(
        { length: TRAINING_TARGET_COUNT },
        (_target, targetIndex) => `target${laneIndex}_${targetIndex}`,
    ),
);

export const TRAINING_TRACKED_DISCS = {
    [TRAINING_TRACKED_DISC.LANE_BALL_0]: TRAINING_BALL_DISC_REF.LANE_0,
    [TRAINING_TRACKED_DISC.LANE_BALL_1]: TRAINING_BALL_DISC_REF.LANE_1,
    [TRAINING_TRACKED_DISC.LANE_BALL_2]: TRAINING_BALL_DISC_REF.LANE_2,
    ...Object.fromEntries(
        TRAINING_TARGET_DISC_REFS.flatMap((targetRefs) =>
            targetRefs.map((targetRef) => [targetRef, targetRef]),
        ),
    ),
} as const;
...
export const { stadium: trainingStadium } = defineStadium({
    name: "Training",
    width: 1090,
    height: 395,
    bg: {
        type: "none",
        width:
            classicMapMeasures.OUTER_FIELD.bottomRight.x -
            classicMapMeasures.OUTER_FIELD.topLeft.x,
        height:
            classicMapMeasures.OUTER_FIELD.bottomRight.y -
            classicMapMeasures.OUTER_FIELD.topLeft.y,
        color: "718C5A",
    },
    cameraFollow: "player",
    canBeStored: false,
